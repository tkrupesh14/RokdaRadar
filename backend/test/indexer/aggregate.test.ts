...
describe("aggregate correctness against a hand-computed fixture", () => {
  beforeEach(() => {
    ...
    ].forEach(([utrHash, donorRef, amountPaise], i) => {
      onDonationAttested({
        txHash: `0xdon${i}`,
        logIndex: 0,
        id: 1,
        utrHash: utrHash as string,
        donorRef: donorRef as string,
    ...
    onDeliveryAttested({ txHash: "0xdeliverA", logIndex: 0, id: 1, spendRef: "0xspendA", attestor: "0xattestor", ts: 1_000_000 + 14400 });
  });

  it("computes raised/spent/unspent correctly", () => {
    const agg = computeAggregate(1)!;
    expect(agg.raisedPaise).toBe(60000);
    expect(agg.spentPaise).toBe(50000);
    expect(agg.unspentPaise).toBe(10000);
  });

  it("computes donation/spend counts", () => {
    const agg = computeAggregate(1)!;
    expect(agg.donationCount).toBe(3);
    expect(agg.spendCount).toBe(3);
  });

  it("computes categorySplit for all six categories, zero-filled", () => {
    const agg = computeAggregate(1)!;
  ...
  it("computes fieldVsAdminRatio = (spent - admin) / spent", () => {
    const agg = computeAggregate(1)!;
    expect(agg.fieldVsAdminRatio).toBeCloseTo((50000 - 10000) / 50000, 3);
  });

  it("computes vendorConcentration sharePct and flags vendor_concentration for vendorA", () => {
    const agg = computeAggregate(1)!;
  ...
  it("flags admin_ratio when admin share exceeds 15%", () => {
    const agg = computeAggregate(1)!;
    expect(agg.anomalyCandidates.some((a) => a.reason === "admin_ratio")).toBe(true);
  });

  it("computes medianDonationPaise", () => {
    const agg = computeAggregate(1)!;
    expect(agg.medianDonationPaise).toBe(20000);
  });

  it("computes deliveryAttestedPct = attested / total spends * 100", () => {
    const agg = computeAggregate(1)!;
    expect(agg.deliveryAttestedPct).toBeCloseTo(100 / 3, 1);
  });

  it("builds txIndex keyed by spendRef and by campaignId for the creation tx", () => {
    const agg = computeAggregate(1)!;
    expect(agg.txIndex["0xspendA"]).toBe("0xspendA");
    expect(agg.txIndex["1"]).toBe("0xcreate");
  });

  it("returns null for a non-existent campaign", () => {
    expect(computeAggregate(999)).toBeNull();
  });
});
